...
describe("attempt connection to rabbitmq broker", () => {
    ...
    it("should produce/consume hundreds messages", async () => {
        Array(256)
            .fill(0)
            .map(async (_, i) =>
                await broker.sendToQueue(broker.openConnection(), `message ${i}`)
            );
        await broker.consumeFromQueue(broker.openConnection(), (msg) => {
